...
func TestConfigManager_ParseConfig_NewFields(t *testing.T) {
	...
	testSettings := map[string]string{
		...
		"polishing_api_key":    "test-polishing-key",
		"polishing_model_name": "gpt-4o",

		// 译文优化配置
		"optimization_enabled":    "true",
		"optimization_provider":   "gemini",
		"optimization_api_key":    "test-optimization-key",
		"optimization_model_name": "gemini-1.5-flash",

		// 声音克隆配置
		"voice_cloning_provider":    "aliyun_cosyvoice",
		"voice_cloning_api_key":     "test-voice-key",
		"voice_cloning_output_dir":  "/tmp/voice-output",

		// 阿里云 OSS 配置
		"aliyun_oss_access_key_id":     "test-oss-key-id",
		"aliyun_oss_access_key_secret": "test-oss-secret",
...
func TestConfigManager_ParseConfig_EncryptedOSSConfig(t *testing.T) {
	...
	// 加密 OSS 凭证
	encryptedKeyID, err := cryptoManager.EncryptAPIKey("test-oss-key-id")
	if err != nil {
		t.Fatalf("EncryptAPIKey() error = %v", err)
	}

	encryptedSecret, err := cryptoManager.EncryptAPIKey("test-oss-secret")
	if err != nil {
		t.Fatalf("EncryptAPIKey() error = %v", err)
	}
...
func TestConfigManager_Cache_NewFields(t *testing.T) {
	...
	// 第一次获取配置（从 Redis 读取）
	appConfig1, err := configManager.GetConfig()
	if err != nil {
		t.Fatalf("GetConfig() error = %v", err)
	}
	...
	// 修改 Redis 中的配置
	redisClient.HSet("app:settings", "asr_language_code", "en-US")

	// 第二次获取配置（从缓存读取，应该还是旧值）
	appConfig2, err := configManager.GetConfig()
	if err != nil {
		t.Fatalf("GetConfig() error = %v", err)
	}
	...
	appConfig3, err := configManager.GetConfig()
	if err != nil {
		t.Fatalf("GetConfig() error = %v", err)
	}
