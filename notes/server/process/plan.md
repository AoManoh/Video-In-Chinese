...
**约束3：开发进度跟踪强制要求**
1. **唯一真实来源**：`notes/server/process/development-todo.md` 是开发进度的唯一真实来源（Single Source of Truth）
2. **实时更新**：每完成一个任务点必须立即更新状态标记
   - 未开始的任务：`- [ ]`
   - 进行中的任务：`- [/]`
   - 已完成的任务：`- [x]`
3. **模块完成标准**：
   - 当某个模块的所有开发任务完成后，在该任务项后标记 `✅ 开发完成`
   - 当该模块的接口/API测试通过后，在模块标题后标记 `✅ 测试通过，模块完成`
4. **测试优先**：每个模块开发完成后必须先通过测试才能标记为完成，不得跳过测试环节
5. **文档同步**：development-todo.md 的更新日志必须记录每次重要的进度变更

**约束4：环境配置默认值清单**（严格遵守第三层文档）

以下为**核心环境变量**（通过.env文件或环境变量配置，修改后需重启服务）。完整的环境变量列表（约54个，包含Gateway 13个、AIAdaptor 13个、AudioSeparator 6个、Processor 11个、Task 11个）请参考各服务的第三层文档（design-detail.md）。
...
| `MAX_UPLOAD_SIZE_MB`              | `2048`           | 文件上传最大大小（MB）                  | Gateway |
...
4. **Processor → Redis**（配置读取）:
   - Processor需要读取应用配置（`app:settings`）
   - 获取audio_separation_enabled、polishing_enabled等配置项
   - 参考：Processor-design.md v2.6 第388-392行"步骤2：读取应用配置"
...
- [ ] 单元测试：Composer包测试
  - 测试音频拼接（使用真实音频文件）
  - 测试时长对齐（4个策略：静音填充、语速加速、LLM重译、截断）
  - 测试音频合并（人声+背景音）
- [ ] 单元测试：Mediautil包测试
  - 测试音频提取（使用真实视频文件）
  - 测试音视频合并
...
- [ ] 单元测试：文件上传测试
  - 测试流式保存（Mock文件流）
  - 测试文件大小检查（MAX_UPLOAD_SIZE_MB=2048）
  - 测试磁盘空间检查
  - 测试MIME Type验证
  - 测试临时文件清理
- [ ] 单元测试：任务状态查询测试
...
- [ ] 异常场景测试通过（磁盘空间不足、Redis连接失败、配置缺失等）
...
| **M3: 核心业务层完成**     | Processor服务通过14步流程端到端测试                                      |
...
- [ ] Gateway可提供文件下载（流式传输）
- [ ] 配置管理功能正常（加密存储+乐观锁）
- [ ] MAX_UPLOAD_SIZE_MB=2048限制正常工作
- [ ] 单元测试覆盖率>80%
- [ ] 端到端测试通过（完整HTTP API流程）

**系统集成验收标准**:
- [ ] 用户可通过Web界面上传视频文件（最大2048MB）
- [ ] 系统可完整执行视频翻译流程（14步）
- [ ] 用户可下载翻译后的视频文件
